// Adds the Material Behavior batch to a local master atlas and writes the
// Material-Behavior-Batch-01-with-prerequisites sub-map beside it.
//
//   node authoring/apply-material-behavior.mjs <master.json>            dry run: report only
//   node authoring/apply-material-behavior.mjs <master.json> --write    write the master and the sub-map
//   --submap <path>   sub-map location (default: Material-Behavior-Batch-01-with-prerequisites.json beside the master)
//
...
import { mergeMaterialBehavior, materialSubmap, clearance, loadBatch } from './material-behavior/merge.mjs';
...
if (!masterPath) { console.error('Usage: node authoring/apply-material-behavior.mjs <master.json> [--write] [--submap <path>]'); process.exit(2); }
const submapPath = option('--submap') || path.join(path.dirname(masterPath), 'Material-Behavior-Batch-01-with-prerequisites.json');
...
const { graph, report } = mergeMaterialBehavior(atlas, batch);
const sub = materialSubmap(graph, batch);
const summary = {
