// Adds the DC Circuits batch to a local master atlas and writes the DC-Circuits-with-prerequisites
// sub-map beside it.
//
//   node authoring/apply-dc-circuits.mjs <master.json>            dry run: report only
//   node authoring/apply-dc-circuits.mjs <master.json> --write    write the master and the sub-map
//   --submap <path>   sub-map location (default: DC-Circuits-with-prerequisites.json beside the master)
//
...
import { mergeDcCircuits, dcSubmap, clearance, loadBatch } from './dc-circuits/merge.mjs';
...
if (!masterPath) { console.error('Usage: node authoring/apply-dc-circuits.mjs <master.json> [--write] [--submap <path>]'); process.exit(2); }
const submapPath = option('--submap') || path.join(path.dirname(masterPath), 'DC-Circuits-with-prerequisites.json');
...
const { graph, report } = mergeDcCircuits(atlas, batch);
const sub = dcSubmap(graph, batch);
const summary = {
  master: masterPath, submap: submapPath, masterSha256: hash(text),
  before, after: { nodes: graph.nodes.length, edges: graph.edges.length, marked: graph.nodes.filter(n => n.proficiency80 != null).length, electronics: graph.nodes.filter(n => n.domain === 'Electronics').length },
