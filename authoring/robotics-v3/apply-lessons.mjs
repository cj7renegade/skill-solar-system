...
import { applyLessons, preservationDiff } from './lessons.mjs';

const ROOT = path.resolve(import.meta.dirname, '..', '..');
const MAP = path.join(ROOT, 'Maps', 'Robotics-v3', 'Robotics-v3-Preview.json');
...
export function loadInputs() {
  ...
  return { spec: read('spec'), edition01: read('edition01'), edition02: read('edition02') };
}
...
const editions = [
  { metadata: inputs.edition01.data.metadata, lessons: inputs.edition01.data.lessons },
  { metadata: inputs.edition02.data.metadata, lessons: inputs.edition02.data.lessons }
];
...
const summary = {
  ...
  expected: { uniqueAuthoredCards: 193, pendingAssessable: 187, roadmapEntries: 1, mapEntries: 381 },
  reconciliation: {
    ...
    nodesGivenContentStatusOnly: report.changed.length - report.applied.length,
  ...
  idempotent: stable,
  limitations: [
    'The lessons are introductory cards. They are not a complete course, an exhaustive question bank or evidence of practical mastery.',
    '187 assessable entries still have no authored introductory lesson, and one roadmap entry is not assessed.',
    'No proficiency answer is read, written or inferred by this import.',
    'The teaching simulation is supplemental and is not wired into the application.',
    'The map title now states the authored and pending counts. A guided-review session saved against the earlier title starts again, because review sessions are keyed on the title and node set. Review order itself is unchanged: it comes from saved heights, names and ids, none of which this patch touches.'
  ]
};

const ok = summary.preservation.reviewOrderUnchanged && !preservation.length && stable && report.counts.imported === 193 && report.counts.authored === 193 && report.counts.pending === 187 && report.counts.roadmap === 1 && answersBefore === answersAfter && atlasFamily(before) === atlasFamily(graph);
