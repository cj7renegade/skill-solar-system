// Adds the Robotics Foundations batch to a local master atlas and writes the
// Robotics-Foundations-Batch-01-with-prerequisites sub-map beside it.
//
//   node authoring/apply-robotics-foundations.mjs <master.json>            dry run: report only
//   node authoring/apply-robotics-foundations.mjs <master.json> --write    write the master and the sub-map
//   --submap <path>   sub-map location (default: Robotics-Foundations-Batch-01-with-prerequisites.json beside the master)
//
...
import { mergeRoboticsFoundations, roboticsSubmap, clearance, loadBatch } from './robotics-foundations/merge.mjs';
...
if (!masterPath) { console.error('Usage: node authoring/apply-robotics-foundations.mjs <master.json> [--write] [--submap <path>]'); process.exit(2); }
const submapPath = option('--submap') || path.join(path.dirname(masterPath), 'Robotics-Foundations-Batch-01-with-prerequisites.json');
...
const { graph, report } = mergeRoboticsFoundations(atlas, batch);
const sub = roboticsSubmap(graph, batch);
const summary = {
  master: masterPath, submap: submapPath, masterSha256: hash(text),
  before, after: { nodes: graph.nodes.length, edges: graph.edges.length, marked: graph.nodes.filter(n => n.proficiency80 != null).length, robotics: graph.nodes.filter(n => n.domain === 'Robotics').length },
