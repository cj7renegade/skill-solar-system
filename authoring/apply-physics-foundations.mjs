// Adds the Physics Foundations batch to a local master atlas and writes the
// Physics-Foundations-Batch-01-with-prerequisites sub-map beside it.
//
//   node authoring/apply-physics-foundations.mjs <master.json>            dry run: report only
//   node authoring/apply-physics-foundations.mjs <master.json> --write    write the master and the sub-map
//   --submap <path>   sub-map location (default: Physics-Foundations-Batch-01-with-prerequisites.json beside the master)
//
...
import { mergePhysicsFoundations, physicsSubmap, clearance, loadBatch } from './physics-foundations/merge.mjs';
...
if (!masterPath) { console.error('Usage: node authoring/apply-physics-foundations.mjs <master.json> [--write] [--submap <path>]'); process.exit(2); }
const submapPath = option('--submap') || path.join(path.dirname(masterPath), 'Physics-Foundations-Batch-01-with-prerequisites.json');
...
const { graph, report } = mergePhysicsFoundations(atlas, batch);
const sub = physicsSubmap(graph, batch);
const summary = {
  master: masterPath, submap: submapPath, masterSha256: hash(text),
  before, after: { nodes: graph.nodes.length, edges: graph.edges.length, marked: graph.nodes.filter(n => n.proficiency80 != null).length, physics: graph.nodes.filter(n => n.domain === 'Physics').length },
