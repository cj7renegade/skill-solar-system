...
// real input events over the Chrome DevTools Protocol.
...
export async function launchApp(prefix) {
  ...
  const click = async (sel, wait = 450) => { const p = await center(sel); assert.ok(p, `missing ${sel}`); await clickAt(p, wait); };
  ...
  return { dir, send, evaluate, waitFor, open, mouse, center, clickAt, click, drag, wheel, key, labels, selectedLabel, sphereOnScreen, canvasCenter, pageErrors, close };
}
