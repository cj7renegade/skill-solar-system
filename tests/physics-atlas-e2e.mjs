// Optional end-to-end check of the integrated Physics Foundations batch in the real Electron app,
// using copies of the local maps in an isolated profile with its own proficiency store, so the
// user's real answers are never touched. Set SSS_ATLAS to the master and SSS_PHYS_SUBMAP to the
...
import { loadBatch } from '../authoring/physics-foundations/merge.mjs';

if (!process.env.SSS_ATLAS || !process.env.SSS_PHYS_SUBMAP) { console.log('Skipped: set SSS_ATLAS and SSS_PHYS_SUBMAP to check the integrated Physics batch.'); process.exit(0); }
const { check, passed } = checker();
const batch = loadBatch();
const app = await launchApp('sss-phys-atlas-');
...
try {
  ...
  const master = path.join(app.dir, 'atlas-master.json'), sub = path.join(app.dir, 'atlas-physics.json');
  copyFileSync(process.env.SSS_ATLAS, master); copyFileSync(process.env.SSS_PHYS_SUBMAP, sub);
  const data = JSON.parse(readFileSync(master, 'utf8')), subData = JSON.parse(readFileSync(sub, 'utf8'));
  const physics = data.nodes.filter(n => n.domain === 'Physics');
  ...
  check('all 19 new skills are present and unmarked in the file', fresh.length === 19 && fresh.every(n => n.proficiency80 === null));
  check('the 26 DC skills integrated earlier are still there', data.nodes.filter(n => n.authoringOrigin === 'sss-dc-batch-01').length === 26);
  check('the Physics legend counts the additions', await evaluate(`document.querySelector('[data-subject="Physics"] .legend-count').textContent`) === String(physics.length), `${physics.length} Physics`);
  await click('#home', 600);
  await click('[data-subject="Physics"]', 400);
  check('highlighting Physics marks every skill in it, the new abilities included', await highlighted() === physics.length);
  await keep('01-physics-overview-highlighted');

  // Find skill reaches the new branch, and the card shows the authored content.
  const target = fresh.find(n => n.id === 'sss-phys-freefall-apex');
  await evaluate(`(()=>{const f=document.getElementById('find-skill');f.focus();f.value='vertical throw';f.dispatchEvent(new Event('input'));})()`); await sleep(400);
  const options = await evaluate(`[...document.querySelectorAll('#find-results button')].map(b=>b.textContent.replace('Physics','').trim()).join('|')`);
  check('Find skill reaches the new Physics branch', options.includes(target.name), options.slice(0, 140));
  ...
  await keep('02-physics-skill-close');
  ...
  check('the skill card shows both paragraphs and where it sits', target.details.split('\n\n').every(p => card.includes(p.slice(0, 60))) && /Physics strand|strand of the spiral/.test(card), card.slice(0, 120));
  check('the card shows the prerequisites the batch recorded', /Free fall near Earth|builds on/.test(card), card.slice(0, 200));
  await keep('03-physics-skill-card');
  ...
  const probe = fresh.find(n => n.id === 'sss-phys-mass-weight'), overview = 'p-weight';
  ...
  await keep('04-physics-submap');
  await open(master);
  check('clearing it in the sub-map clears it in the master', await value(probe.id) === null);
  check('the copied originals were not changed', readFileSync(master, 'utf8') === readFileSync(process.env.SSS_ATLAS, 'utf8') && readFileSync(sub, 'utf8') === readFileSync(process.env.SSS_PHYS_SUBMAP, 'utf8'));
  check('no uncaught page errors', pageErrors.length === 0, pageErrors.join(' | '));
  console.log(`\n${passed.length} Physics integration checks passed.`);
} catch (error) {
