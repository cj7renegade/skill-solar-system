// Optional end-to-end check of the integrated Material Behavior batch in the real Electron app,
// using copies of the local maps in an isolated profile with its own proficiency store, so the
// user's real answers are never touched. Set SSS_ATLAS to the master and SSS_MAT_SUBMAP to the
...
import { loadBatch } from '../authoring/material-behavior/merge.mjs';

if (!process.env.SSS_ATLAS || !process.env.SSS_MAT_SUBMAP) { console.log('Skipped: set SSS_ATLAS and SSS_MAT_SUBMAP to check the integrated Material Behavior batch.'); process.exit(0); }
const { check, passed } = checker();
const batch = loadBatch();
const app = await launchApp('sss-mat-atlas-');
...
try {
  ...
  const master = path.join(app.dir, 'atlas-master.json'), sub = path.join(app.dir, 'atlas-material.json');
  copyFileSync(process.env.SSS_ATLAS, master); copyFileSync(process.env.SSS_MAT_SUBMAP, sub);
  ...
  check('all 10 new skills are present and unmarked in the file', fresh.length === 10 && fresh.every(n => n.proficiency80 === null));
  check('the batches integrated earlier are still there', data.nodes.filter(n => n.authoringOrigin === 'sss-dc-batch-01').length === 26 && data.nodes.filter(n => n.authoringOrigin === 'sss-physics-foundations-batch-01').length === 19);
  ...
  await keep('01-material-overview-highlighted');

  // Find skill reaches the new branch, and the card shows the authored content.
  const target = fresh.find(n => n.id === 'sss-mat-proof-stress');
  await evaluate(`(()=>{const f=document.getElementById('find-skill');f.focus();f.value='proof stress';f.dispatchEvent(new Event('input'));})()`); await sleep(400);
  ...
  check('Find skill reaches the new Material Behavior branch', options.includes(target.name), options.slice(0, 140));
  ...
  await keep('02-material-skill-close');
  ...
  check('the card shows the prerequisites the batch recorded', /Young|modulus|Yielding|builds on/.test(card), card.slice(0, 200));
  await keep('03-material-skill-card');
  ...
  const probe = fresh.find(n => n.id === 'sss-mat-engineering-stress'), overview = 's-stress';
  ...
  await keep('04-material-submap');
  await open(master);
  check('clearing it in the sub-map clears it in the master', await value(probe.id) === null);
  check('the copied originals were not changed', readFileSync(master, 'utf8') === readFileSync(process.env.SSS_ATLAS, 'utf8') && readFileSync(sub, 'utf8') === readFileSync(process.env.SSS_MAT_SUBMAP, 'utf8'));
  check('no uncaught page errors', pageErrors.length === 0, pageErrors.join(' | '));
  console.log(`\n${passed.length} Material Behavior integration checks passed.`);
} catch (error) {
